...
case "$(ps h -o comm 1)" in
  ...
  runit)
    ...
    runit_ln() {
      ...
      ln -sfn "$service_dir" "$active_link" \
      ...
      if ! run_step "Starting auto-cpufreq daemon (runit) service" sv start "$active_link"; then
        # Keep the activation symlink in place so the removal helper can
        # synchronously confirm sv stop before rollback restores host services.
        exit 1
      fi
    }
  ...
  systemd)
    ...
    run_step "Reloading systemd manager configuration" systemctl daemon-reload \
      || exit 1
    run_step "Starting auto-cpufreq daemon (systemd) service" systemctl start auto-cpufreq.service \
      || exit 1
    if ! run_step "Enabling auto-cpufreq daemon (systemd) at boot" systemctl enable auto-cpufreq.service; then
      rollback_failed=0

      run_step "Rolling back auto-cpufreq daemon (systemd) service" systemctl stop auto-cpufreq.service \
        || rollback_failed=1
      run_step "Rolling back auto-cpufreq daemon (systemd) boot activation" systemctl disable auto-cpufreq.service \
        || rollback_failed=1

      if [ "$rollback_failed" -ne 0 ]; then
        echo "Error: Failed to fully roll back the partial systemd installation." >&2
      fi
      exit 1
    fi
  ;;
  ...
  s6-svscan)
    ...
    mkdir -p /etc/s6/sv/auto-cpufreq \
      || fail_install "Failed to create the s6 service directory."
    cp -r /usr/local/share/auto-cpufreq/scripts/auto-cpufreq-s6/. /etc/s6/sv/auto-cpufreq/ \
      || fail_install "Failed to deploy the s6 service definition."

    if command -v s6 > /dev/null 2>&1; then
      # s6-frontend treats /etc/s6/sv as a service store. Synchronize the
      # repository after adding the definition, enable the newly imported
      # service in the working set, then commit and install that set before
      # asking the live database to start auto-cpufreq.
      run_step "Synchronizing the s6 service repository" s6 repository sync \
        || exit 1
      run_step "Enabling auto-cpufreq daemon (s6) at boot" s6 set enable auto-cpufreq \
        || exit 1
      ...
      run_step "Installing the updated s6 live database" s6 live install \
        || exit 1
      run_step "Starting auto-cpufreq daemon (s6) service" s6 live start auto-cpufreq \
        || exit 1
    else
      # Older Artix releases used distribution-specific wrappers around s6-rc.
      ...
      run_step "Adding auto-cpufreq service (legacy s6) to default bundle" \
      ...
      if ! run_step "Starting auto-cpufreq daemon (legacy s6) service" \
        s6-rc -u change auto-cpufreq; then
        s6-service delete default auto-cpufreq > /dev/null 2>&1 || true
